...
def f06(string1, string2):
    ...
    s1 = float(len(strings[0]))
    s2 = float(len(strings[1]))
    size = (s2/s1)*100

    print"{0} is longer than {1} by {2} chars.".format(strings[0], strings[1], difference)
    print"{0} has only {1} the number of chars of {2}".format(strings[1], size, strings[0])
...
"""def f12():
    ...
    with open("log_file.txt", 'w') as f:
        while True:
            try:
                good_input.append(float(raw_input("Enter input:\n")))
            except:
                f.write
                else:
                    f.write(user_in)
...
# [[], [], [], [], [], [], [], [], [], [0]]
def f15():
    # This is a pretty hokey solution
    goal = [[0], [],[],[],[],[],[],[],[],[]]
    print goal
...
def f17():
    ...
    list_ =  zip(indice, words)


    # Be sure to save the list that you print to list_
    list_ =  list_  # Change to your list 
